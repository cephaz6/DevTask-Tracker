...
from utils.dashboard import get_admin_dashboard_data, get_project_dashboard_data, serialize_task, get_user_dashboard_data
...
def get_dashboard(
    ...
    try:
        ...
        try:
            ...
            owned_projects = session.query(Project).filter(Project.owner_id == current_user.id).all()
        ...
        try:
            ...
            member_projects = session.query(ProjectMember).filter(
                ProjectMember.user_id == current_user.id,
                ~ProjectMember.project_id.in_(owned_project_ids)
            ).all()
        except Exception as e:
            # log error e
            raise HTTPException(status_code=500, detail="Error fetching project memberships")
        ...
        try:
            for project in owned_projects:
                proj_data = get_project_dashboard_data(session, project, owner_view=True)
                response["owned_projects"].append(proj_data)
        except Exception as e:
            # log error e
            raise HTTPException(status_code=500, detail="Error assembling owned projects dashboard")

        try:
            for pm in member_projects:
                project = session.query(Project).get(pm.project_id)
                personal_tasks = session.query(Task).filter(
            ...
            # log error e
            raise HTTPException(status_code=500, detail="Error assembling member projects dashboard")
...
def project_dashboard(
    ...
    try:
        ...
        return get_project_dashboard_data(session=session, project=project, owner_view=is_owner)
